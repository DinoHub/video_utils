...
video_feed_names = os.environ.get("VIDEO_FEED_NAMES", "FILE1,RTSP2")
...
manual_video_fps = os.environ.get(
    "MANUAL_VIDEO_FPS", "-1,-1"
)  # -1 to try to read from video stream metadata
...
def one_video_manager_to_many_source():
    ...
    vidManager = VideoManager(
        video_feed_names=video_feed_names.split(","),
        streams=streams.split(","),
        source_types=source_types.split(","),
        manual_video_fps=manual_video_fps.split(","),
    ...
    for frame_count in itertools.count():
        frame_of_each_video_feed = (
            vidManager.read()
        )  # frames is list of arrays from 0 - 255, dtype uint8
        for i, video_stream_information in enumerate(vidManager.videos):
            if len(frame_of_each_video_feed[i]) != 0:
                drawn_frame = frame_drawer.draw_detections(
                    frame_of_each_video_feed[i],
                    [
